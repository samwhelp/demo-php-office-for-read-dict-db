...
class SaveAsOds {
	...
	public function __construct () {
		...
		// 設定欄位名稱
		$sheet->setCellValue('A1', '主要');
		$sheet->setCellValue('B1', '承接');
	...
	public function run () {
		...
		if (!$this->_TargetFilePath) {
			$this->_TargetFilePath = THE_CLI_VAR_DIR_PATH . '/result/link-base.ods';
		}
	...
	protected function syncData () {
		...
		foreach ($this->_Meta as $row => $list) {
	...
	protected function fixData () {
		...
		foreach ($this->_Data as $main => $list) {
			foreach ($list as $index => $atom) {
				$unit = array();
				$unit[0] = $main;
				$unit[1] = $atom;
				$data[] = $unit;
